...
public interface IWorkspaceService
{
    ...
    Task<WorkspaceResponseDto> UpdateWorkspaceAsync(
        Guid id,
        UpdateWorkspaceRequest dto,
        CancellationToken ct = default
    );
    Task DeleteWorkspaceAsync(Guid id, CancellationToken ct = default);
    Task<IEnumerable<WorkspaceResponseDto>> GetDeletedWorkspacesAsync(
        CancellationToken ct = default
    );
    Task<WorkspaceResponseDto> RestoreWorkspaceAsync(Guid id, CancellationToken ct = default);

    Task<IEnumerable<WorkspaceMemberResponseDto>> GetMembersAsync(
        Guid id,
        CancellationToken ct = default
    );
    Task<WorkspaceMemberResponseDto> AddMemberAsync(
        Guid id,
        AddMemberRequest dto,
        CancellationToken ct = default
    );
    Task<WorkspaceMemberResponseDto> ChangeRoleAsync(
        Guid id,
        Guid userId,
